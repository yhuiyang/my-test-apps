...
/**
 * <p>
 * The instance of <code>GuessTreeNode</code> represents a state of the guessing
 * game.
 * </p>
 * 
 * <p>
 * Consider the case where the number of digits is four, there will be 5040
 * possible numbers at the begining. A new constructed node represents the
 * initial state, we also call this node as the root of the game tree. The size
 * of the node is defined as the number of possible numbers. Therefore, the size
 * of the root node is 5040. After the first guess, for example, gussing "1234",
 * there will be 14 different responses from the user, i.e., 0A0B, 0A1B, 0A2B,
 * 0A3B, 0A4B, ....., 3A0B, 4A0B. Except for the case of 4A0B, we use different
 * nodes for each different case. We say these 13 nodes are the children of the
 * root node and the guessing number, i.e, 1234, is the <code>splitGuess</code>
 * of the root node. And we say the root node is split. On the contrary, these
 * 13 children are unsplit at this moment.
 * </p>
 * 
 * <p>
 * If the game tree has split in all node whose sizes are large than one. We say
 * this tree is a COMPLETE game tree, because it has simulated all the cases of
 * the users response. We can play the game with such complete tree. This is
 * also the reason why this class has implemented the <code>Serializable</code>
 * interface and can be serialized into permanent storage. You can use <code>
 * GuessTreeChooser</code> to use a tree to play a game.
 * </p>
 * 
...
public class GuessTreeNode implements Serializable {
     ...
     * IllegalStateException</code> will be thrown if this node has not been
     * split yet.
     * 
     ...
     * <code>IllegalStateException</code> will be thrown if this node has not
     * been split yet. If the index is not in the range of [0, childCount), an
     * exception of <code>ArrayIndexOutOfBoundsException</code> will be thrown.
     * 
     * @param index
     *            the index of the node required
    ...
    /**
     * Gets the parent node of this node. If this node is the root of the game
     * tree, <code>null</code> will be returned.
     * 
     * @return the parent node of this node or <code>null</code> if this node is
    ...
    private int organize(int target, int s, int e) {
        ...
        if (--e < s)
            return e + 1;
        while (true) {
            while (s < e && label[s] == target)
                ++s;
            while (s < e && label[e] != target)
                --e;
            if (s >= e)
                break;
    ...
    /**
     * <p>
     * Gets the possible candidate of the given index. The order of these
     * possible candidates will be changed when any descendant of this node is
     * split/resplit.
     * </p>
     * 
     * <p>
     * The returned number is in HEX based. That is if the number is "0123" the
     * value is 0x0123 which is 291 in decimal format.
     * </p>
     * 
     * @param index
     *            the index of the returned candidate
     * @return the possible candidate of the given index
     * @throws ArrayIndexOutOfBounds
     *             if the given index is not in the range of [0, size()).
     */
    ...
    /**
     * <p>
     * Gets all the possible candidates of this node. The returned
     * <code>int</code> array is created inside the function and is independent
     * from the instance.
     * </p>
     * 
     * <p>
     ...
     * 
     * @return <code>true</code> if this node is split; <code>false</code>
     *         otherwise.
     */
    ...
    /**
     * <p>
     * Gets the total depth of all candidate numbers in this node. The depth of
     * a candidate number is defined as the depth of the node where the
     * <code>splitNumber</code> is the candidate. The depth is counted from this
     * node (not root) and is started from 1.
     * </p>
     * 
     * <p>
     * The depth of a candidate is also known as the number of guesses need to
     * catch the candidate.
     * </p>
     * 
     * <p>
     * For example, assume the candidate numbers in this node are "0123", "0124"
     * and "0142", and the split guess of this node is "0123". Under such
     * assumption, this node has two child nodes. Then the total cost will be 1
     * + 2 + 2 = 5.
     * </p>
     * 
     * <p>
     * This function can be called only if all the descendant have been split.
     * Otherwise, an <code>IllegalStateException</code> will be thrown.
     * </p>
     * 
    ...
    /**
     * Gets the depth of the node. The depth is started from root and is
     * counting from 1. For example the depth of the root node is 1.
     * 
     ...
     * splitGuess</code>s of the ancestors of this node. The returned guesses
     * will follow the order of the path, i.e., the <code>splitGuess</code> will
    ...
    /**
     * Gets the digit mask of all the guessed numbers. If one digit is used in
     * these guessed numbers the corresponding bit in the digit mask will be
     * set. For example, if the guessed numbers is "0123", then the mask will be
     * 0x000F. If the guessed numbers are "0123" and "4501", the bit mask will
     * be 0x003F.
     * 
    ...
    /**
     * <p>
     * Gets the statisitics on the depthes of all the candidates of this node.
     * Please refer to <code>getTotalDepth()</code> for the definition of the
     * depth of a candidate.
     * </p>
     * 
     * <p>
     * The returned <code>int</code> array represents the number of candidates
     * of different depthes. For example, if the returned int array is { 1, 3, 5
     * }, that means there are 1 candidate is of depth 1 is 1, and 3 candidates
     * are of depth 2, and 5 candidate are of depth 3.
     * </p>
     * 
    ...
    /**
     * <p>
     * Finds the child of the specified label. In the game, the "label" is the
     * response from users to indicate the relation between guess and the
     ...
     * 
     * <p>
     * After spliting, all the candidates of the same label will be grouped as a
     * child node. Thus, we define the label of these candidate as the label of
     * the child node.
     * </p>
     * 
     * <p>
     * If there is no such child node of such label, <code>null
    ...
    /**
     * <p>
     * Performs the label statistics over the all candidates in this node
     * according to the given guess number. For the performance issue, client
     * should prepare an int array of length at least 0x100(256) before calling
     * this function. Otherwise, an <code>IllegalArgumentException</code> will
     * be thrown.
     * </p>
     * 
     * <p>
     * After calling this function, the statistics result will be stored in the
     * array <code>stat</code>. For example, if the value of
     * <code>stat[0x10]<code> is 7, that means there are 
    ...
    /**
     * Performs the digit statistics over all the candidates in this node. For
     * the performance issue, clients should prepare a two layered
     * <code>int</code> array of dimension of
     * <code>int[digitCount][symbolCount]</code> before calling this function.
     * After calling this function, the statistics result will be stored in the
     * <code>stat</code> array. The value of stat[i][j] means the number of
     * candidates where the digit 'i' appears in the position 'j'.
     * 
     * @param stat
     *            the space to store the statistic result
    ...
    /**
     * Gets the digit mask of all the candidates in the node. Please reference
     * <code>getGuessedDigitMask()</code> for the definition of digit mask. For
     * example, if the candidate numbers is "0123", then the mask is 0x000F. If
     * the candidate numbers is "0123" adn "4501" then the mask will be 0x003F.
     * 
     * @return the digit mask of all the candidate numbers in the node
    ...
    /**
     * Gets the label of the node. After spliting, all the candidate numbers of
     * the same label will be grouped as a child of the split node. The same
     * label of these candidates is also defined as the label of the child node.
     * By this definition, the root node has no label and calling this function
    ...
    /**
     * Gets the index of the child. If the given node is not a child of this
     * node, -1 will be returned
     * 
     * @param node
     *            the node whose index will be returned
     * @return the index of the child, or -1 if the given node is not a child of
     *         this node
     */

...
/**
 * <p>An <code>InfoGainChooser</code> uses the information gain as
 * the score of the guess. Assume the number of candidates before
 * the guess is n, and the guess splits the node into m child nodes. 
 * The sizes of these child nodes are n1, n2, ..., nm, respectively. 
 * Then the information gain is defined as <blockquote><pre>
 * info_gain = n*log(n) - n1*log(n1) - ... - nm*log(nm) 
 * </pre></blockquote> </p>
 * 
 * <p>However, for difference gusesses, the size of the node is the 
 * same, thus, the formula can be simplified as <blockquote><pre>
 * info gain = -n1*log(n1) - n2*log(n2) - ... - nm*log(nm)
 * </pre></blockquote>. But if ni is one, then the cost of that 
 * group will be zero, i.e., no cost. In order to give a differnce 
 * score between a child of size one and the correct guess, we 
 * slidly change the formula to <blockquote><pre>
 * info gain = -n1*(log(n1)+w)-n2*(log(n1)+w)-...
 * </pre></blockquote> 
 * Where the "w" is the panilty of going to the next level, 
 * experiments show that the value "1.8" is a good one for this 
 * parameter.</p>
 * 
 * <p>This formula can also explained as an estimator of the total 
 * depth of all the candidates. The avarage guess times of this 
 * chooser is about 5.24.</p>
 *
